...
def dncnn(input, is_training=True, output_channels=1):
    ...
    #return input - output
    #return tf.tanh(tf.divide(input, output))
    return output
...
class denoiser(object):
    def __init__(self, sess, input_c_dim=1, sigma=25, batch_size=128):
        ...
        #noise = tf.random_normal(shape=tf.shape(self.Y_), stddev=self.sigma / 255.0)

        #self.X = self.Y_ + noise # noisy images     
        ...
        self.Y_noise = dncnn(self.X, is_training=self.is_training)
        self.Y = self.X / self.Y_noise


        self.loss = (1.0 / batch_size) * tf.nn.l2_loss(self.noise - self.Y_noise)
        #self.loss = (1.0 / batch_size) * tf.nn.l2_loss(noise - self.predict_noise)
